import React, { Component } from 'react';
import {
  StyleSheet,
  Text,
  Image,
  View,
  Dimensions,
  Alert,
  TextInput,
  Platform,
//  Modal,
  TouchableOpacity,
} from 'react-native';
var deviceWidth = Dimensions.get('window').width;
var deviceHight = Dimensions.get('window').height;
var base64 = require('base-64');
import Share, {ShareSheet, Button} from 'react-native-share';
import { NavigationActions } from "react-navigation";
var RNFetchBlob = require('react-native-fetch-blob').default
import Ads from "./ads";
import ImageZoom from 'react-native-image-pan-zoom';
import Modal from 'react-native-modal'
const REACT_ICON = 'data:image/png;base64,';

import {
  AdMobBanner,
  AdMobInterstitial,
  PublisherBanner,
  AdMobRewarded,
} from 'react-native-admob'
export default class App extends Component {

    constructor(props) {
        super(props); 
this.state = {
    image:'',
    size:'',
    imageBase64:'',
    visible:false
}
    }

    componentWillMount() {
      this.getSizeImage()
      //this.deleteFile()
      }

 static navigationOptions = {
    title: 'Welcome',
header: null
  };
 
  getSizeImage() {
    const { resultImage } = this.props.navigation.state.params;
    const { status } = this.props.navigation.state.params;
    if(status==="2")
    {

      //alert(resultImage.uri)
      try{
        RNFetchBlob.fs.readFile(resultImage.path, 'base64')
        .then((data) => {
          this.setState({imageBase64:data})
         // alert(data)
          var decodedData = base64.decode(data);
          var bytes = decodedData.length;
          if (bytes < 1024) 
          this.setState({size:bytes + " Bytes"})
          else if (bytes < 1048576)
          this.setState({size:(bytes / 1024).toFixed(3) + " KB"})
          else if (bytes < 1073741824) 
          //alert("MB:" + (bytes / 1048576).toFixed(2) + " MB");
          this.setState({size:(bytes / 1048576).toFixed(3) + " MB"})
          else 
        //  alert((bytes / 1073741824).toFixed(3) + " GB");
          this.setState({size:(bytes / 1073741824).toFixed(3) + " GB"})
        })
      }
        catch(err)
        {
          alert(err)
        }
  
     

    }
    else
    {
     // alert(resultImage.uri)
      try{
        RNFetchBlob.fs.readFile(resultImage.uri, 'base64')
        .then((data) => {
          this.setState({imageBase64:data})
          //alert(data)
          var decodedData = base64.decode(data);
          var bytes = decodedData.length;
          if (bytes < 1024) 
          this.setState({size:bytes + " Bytes"})
          else if (bytes < 1048576)
          this.setState({size:(bytes / 1024).toFixed(3) + " KB"})
          else if (bytes < 1073741824) 
          //alert("MB:" + (bytes / 1048576).toFixed(2) + " MB");
          this.setState({size:(bytes / 1048576).toFixed(3) + " MB"})
          else 
        //  alert((bytes / 1073741824).toFixed(3) + " GB");
          this.setState({size:(bytes / 1073741824).toFixed(3) + " GB"})
        })
      }
        catch(err)
        {
          alert(err)
        }
  
     
    }
    
    
      }

      deleteFile() {
        const { images } = this.props.navigation.state.params;
        const { active } = this.props.navigation.state.params;
        if(active){

        let path = images.uri;
        let new_path = path.substring(path.indexOf('s'))
        RNFetchBlob.fs.unlink("/" + new_path)
          .then(() => {
             var asd = "/" + new_path
            RNFetchBlob.fs.scanFile([{ path: asd, mime: asd.mime }])
           // this.setState({deleteModalVisible:false})
           // this.setState({active:false})
          })
          .catch((err) => { alert(err) })
        }
        else
        {
        //  alert("asd")
        }
      }

      onCancel() {
       // console.log("CANCEL")
        this.setState({visible:false});
      }
      onOpen() {
       // console.log("OPEN")
        this.setState({visible:true});
      }
      choosePhoto()
      {
        
        this.props.navigation.navigate('Home')
      }

  render() {
    const { resultImage } = this.props.navigation.state.params;
    const { navigate } = this.props.navigation;   
    let shareOptions = {
      title: "React Native",
      message: "",
      url: "data:image/png;base64,"+this.state.imageBase64,
      subject: "Share Link" //  for email
    };
        return (
      <View style={styles.container}>
            <Text style={{height:deviceHight*.05,top:deviceHight*.04,fontWeight: "bold",color:"green",fontSize:deviceWidth*.05}}>Image saved successfully</Text>

      <Modal style={{alignItems:"center"}} onBackdropPress={() => this.setState({ visible: false })} isVisible={this.state.visible}>
      <TouchableOpacity onPress={() => this.setState({visible:false})} style={styles.close} >
        <Image source={{uri:'close'}}  style={{width:deviceWidth*.05, height:deviceWidth*.05 }}/>
        </TouchableOpacity>
      <ImageZoom cropWidth={Dimensions.get('window').width}
                       cropHeight={Dimensions.get('window').height}
                       imageWidth={deviceWidth}
                       imageHeight={deviceHight}>
                <Image style={{width:deviceWidth, height:deviceHight}}
                resizeMode='contain'
                       source={resultImage}/>
            </ImageZoom>
        </Modal>
            <View style={{ width: deviceWidth * .81, height: deviceHight * .5,top:deviceHight*.02}}>
            <TouchableOpacity 
             onPress={()=>{this.setState({visible:true}) }} >
            <Image  style={{ width: deviceWidth * .81, height: deviceHight * .4,top:deviceHight*.05,borderRadius:10}} source={resultImage} />
            </TouchableOpacity >
            </View>
{/*       
<Text style={styles.blockText}>photo uri :{cropImage.uri}</Text>
<Text style={styles.blockText}>photo width :{cropImage.width}</Text>
<Text style={styles.blockText}>photo height :{cropImage.height}</Text>
<Text style={styles.blockText}>photo size :{this.state.size}</Text> */}
<Text style={styles.blockText}>Share the photo</Text>
<View style={{flexDirection:"row"}}>

 <TouchableOpacity 
             //onPress={()=>{Share.open(shareOptions);}}
            // onPress={this.onOpen.bind(this)}
            onPress={()=>{
              this.onCancel();
                Share.shareSingle(Object.assign(shareOptions, {
                  "social": "facebook"
                }));
            }}
               activeOpacity={ 0.75 } style={styles.ShareIcon} >
               <Image style={{width:deviceWidth*.1,height:deviceHight*.05}} resizeMode='contain' source={{uri: 'icn_facebook'}} />

               </TouchableOpacity>

               <TouchableOpacity 
             //onPress={()=>{Share.open(shareOptions);}}
             onPress={()=>{
              this.onCancel();
                Share.shareSingle(Object.assign(shareOptions, {
                  "social": "twitter"
                }));
                 }}
                activeOpacity={ 0.75 } style={styles.ShareIcon} >
               <Image style={{width:deviceWidth*.1,height:deviceHight*.05}} resizeMode='contain' source={{uri: 'twitter'}} />

               </TouchableOpacity>

               {/* <TouchableOpacity 
             //onPress={()=>{Share.open(shareOptions);}}
             onPress={()=>{
              this.onCancel();
                Share.shareSingle(Object.assign(shareOptions, {
                  "social": "email"
                }));
            }}
               activeOpacity={ 0.75 } style={styles.ShareIcon} >
               <Image style={{width:deviceWidth*.1,height:deviceHight*.05}} resizeMode='contain' source={{uri: 'icn_google'}} />

               </TouchableOpacity> */}

               <TouchableOpacity 
             //onPress={()=>{Share.open(shareOptions);}}
             onPress={()=>{
              this.onCancel();
                Share.open(shareOptions)
            }}
               activeOpacity={ 0.75 } style={styles.ShareIcon} >
               <Image style={{width:deviceWidth*.1,height:deviceHight*.05}} resizeMode='contain' source={{uri: 'icn_share'}} />

               </TouchableOpacity>


   </View>

{/* <View style={styles.saveView}>

            <TouchableOpacity 
            onPress={()=>{
          Share.open(shareOptions);
        }} 
                          activeOpacity={ 0.75 }
                           style={styles.goToWeb} >
                     <Text  style={styles.ButtonText}>share</Text>
                
      </TouchableOpacity>
      </View> */}
      <View style={styles.saveView}>

            <TouchableOpacity 
            onPress={ () => this.choosePhoto()}  
                          activeOpacity={ 0.75 }
                           style={styles.goToWeb} >
                     <Text  style={styles.ButtonText}>More Photos</Text>
                
      </TouchableOpacity>
      </View>
            <Ads/>
            </View>
    );
  }
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: 'white',
  },
  goToWeb:
  {
      alignItems:'center',
      backgroundColor: "#ef5d5d",
      width:deviceWidth*.577,
      height:deviceHight*.07,
      alignItems:"center",
      justifyContent:"center",
  borderRadius:5,
  },
  ButtonText:
  {
      color:'#ffffff',
      textAlign:'center',
      fontSize:deviceWidth*.05,
      // height:deviceHight*.04,
      // width:deviceWidth*.5,
  },
  saveView:
  {
      height:deviceHight*.1,
      alignItems:"center",
      justifyContent:"center"
  },
  blockText: {
    //marginBottom: 10,
    color:'#5a5959',
    width:deviceWidth*.8,
    height:deviceHight*.08,
    fontSize:deviceWidth*.05,
    textAlign:"center"
  },
  ShareIcon:{
    width:deviceWidth*.2,
    height:deviceHight*.1,
    marginLeft:deviceWidth*.03
  },
  modalViewResize:
  {
    backgroundColor: 'white',
    padding: 22,
    width:deviceWidth*.8,
    height:deviceHight*.8,
    justifyContent: 'center',
    alignItems: 'center',
    borderRadius: 4,
    borderColor: '#ef5d5d',

  },
  close:
  {
   
    position: 'absolute',
    height:deviceWidth*.1,
    width:deviceWidth*.1,
    left:deviceWidth*.85,
    top:0,
    zIndex:100000
  },
});
